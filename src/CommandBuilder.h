...
{
public:
  CommandBuilder& CreateSystem(CmdSystemId id);
  CommandBuilder& CreateStream(CmdStreamId id);
  CommandBuilder& CreateRtc(CmdRtcId id);
  CommandBuilder& CreateNotification(CmdNotificationId id);

  CommandBuilder& Append(uint8_t value);
  CommandBuilder& Append(uint16_t value);
  CommandBuilder& Append(uint32_t value);
  
  Command& Build();

private:
  CommandBuilder& Create(CommandType type, uint8_t id);
  Command command;
};
